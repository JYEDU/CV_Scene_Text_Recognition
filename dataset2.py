...
class LmdbDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            root = self.root.replace('/',os.path.sep)
            dataset_name = root.split(os.path.sep)[-2]
            label_id = dataset_name + '@' + label_key.decode('utf-8')

        return (img, label, label_id)
...
class AlignCollate(object):
    ...
    def __call__(self, batch):
        ...
        images, labels, label_keys = zip(*batch)
        ...
        return image_tensors, labels, label_keys
